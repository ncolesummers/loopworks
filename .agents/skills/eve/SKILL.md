...
Start with `node_modules/eve/docs/README.md` from the repository root. It contains the full
index and recommended reading order. Before writing any eve code, read the
relevant guide there first.

If `eve` is not installed yet, install it (`bun add eve`) or scaffold a new
agent with `bunx eve init <agent-name>`, then read the bundled docs.
