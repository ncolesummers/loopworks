...
Make all three union arms mutually exclusive with optional `never` properties
for fields owned by the other arms. Ordinary discriminated unions reject
conflated fresh object literals, but can admit extra properties from non-fresh
values. Without these exclusions, property-presence narrowing such as
`"reason" in state` could produce a false positive for a non-fresh onboarding
value that also carries `reason`.
...
activated states. The `?: never` exclusions make that separation hold for
non-fresh values as well as fresh object literals.
...
activation. Its non-fresh assignability tests prove that onboarding cannot
carry an unavailable reason and unavailable cannot carry an onboarding stage;
removing the `?: never` exclusions makes their `@ts-expect-error` directives
unused and fails typecheck.
